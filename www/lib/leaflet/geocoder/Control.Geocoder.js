(function (factory) {
	...
	L.Control.Geocoder = L.Control.extend({
		options: {
			...
			errorMessage: 'Aucune correspondance.'
		},
		...
		markGeocode: function(result) {
			...
			this._geocodeMarker = new L.Marker(result.center)
				.bindPopup(result.name + ' <br/> <a href="location/map/add/' 
                                            + result.center.lat + '/' + result.center.lng + '/' 
                                            + '13/' + result.name +'/" class="btn btn-primary btn-xs" role="button"><span style="color:#fff;">Créer une carte</span></a>')
				.addTo(this._map)
				.openPopup();
		...
		_createAlt: function(result, index) {
			var li = document.createElement('li'),
			    ...
			    text = document.createTextNode(result.name + ' <br/> <a href="location/map/add/' 
                                            + result.center.lat + '/' + result.center.lng + '/' 
                                            + '13/' + result.name +'/" class="btn btn-primary btn-xs" role="button"><span style="color:#fff;">Créer une carte</span></a>');
